/**
 * Adapter call — fetch autocomplete suggestions through the
 * `@dashseller/geo` factory. Generates a fresh session token and
 * prints it so you can reuse it with `get-place-details.ts`.
 *
 * Usage:
 *   bun run packages/geo/sandbox/google/adapter/suggest.ts "<query>" [country,country,...]
 *
 * Examples:
 *   bun run packages/geo/sandbox/google/adapter/suggest.ts "1600 amph" us
 *   bun run packages/geo/sandbox/google/adapter/suggest.ts "rue de la" fr,be
 */
...
if (!query) {
  console.error(
    'Example: bun run packages/geo/sandbox/google/adapter/suggest.ts "1600 amph" us'
  );
  process.exit(1);
}
...
const autocomplete = createAutocomplete("google", getConfig());
...
console.log(`\nWrote ${outputFile}`);
console.log(
  `\nResolve a suggestion:\n  bun run packages/geo/sandbox/google/adapter/get-place-details.ts <placeId> ${sessionToken}`
);
process.exit(0);
