 ...
 * `PlaceDetails` via the `@dashseller/geo` factory. Closes the
 * billed Autocomplete session when paired with the sessionToken
 * used by the preceding `suggest` calls.
 *
 * Usage:
...
if (!placeId) {
  console.error(
    "Example: bun run packages/geo/sandbox/google/adapter/get-place-details.ts ChIJj61dQgK6j4AR4GeTYWZsKWw"
  );
  process.exit(1);
}
