 ...
 *   - scan_listing_variant_snapshot  per-unit price and stock history
 ...
 *
 * Both history tables record changes, not scans: a scan that observes the same
 * numbers writes no row. Scan frequency therefore costs storage only when
 * something actually moved, and every history row is a transition.
 *
 * Product identity is split by scope: listing-wide descriptive specifics
 * (brand, manufacturer, and the raw `specifics` bag) sit on the listing, while
 * the identifiers that name one physical unit (UPC/EAN/GTIN/ISBN/MPN/model)
 * sit on the variant, because that is the grain products are matched at.
 */
...
export const scanListing = pgTable(
  "scan_listing",
  {
    ...
    // Product identity — promoted out of `specifics` because research queries
    // filter and group on them. Everything else stays in the bag.
    brand: text("brand"),
    manufacturer: text("manufacturer"),
    /**
     * Every listing-level item specific the marketplace surfaced, name → value
     * (eBay `sellerSpecifiedAspect[]` where `sameValueForAllItemVariations`).
     * Kept whole so a new aspect never needs a migration; the promoted columns
     * above and the identifiers on the variant are copies, not moves.
     */
    specifics: jsonb("specifics").$type<Record<string, string>>(),

    /** Listing format, lowercase: "fixed_price" | "auction". */
    type: text("type"),
    ...
    /** Full-text search over title + brand (generated by PostgreSQL). */
    search: tsvector("search").generatedAlwaysAs(
      (): SQL => sql`
        setweight(to_tsvector('english', coalesce(${scanListing.title}, '')), 'A') ||
        setweight(to_tsvector('english', coalesce(${scanListing.brand}, '')), 'B')
      `
    ),
  },
  (t) => [
    ...
    index("scan_listing_brand_idx").on(t.brand),
    index("scan_listing_search_idx").using("gin", t.search),
  ]
);
...
export const scanListingVariant = pgTable(
  "scan_listing_variant",
  {
    ...
    sku: text("sku"),
    /**
     * Option axis only — name → value of the choices that distinguish this
     * unit from its siblings ({"Color": "Red"}). Descriptive specifics that
     * apply to the whole listing live on `scan_listing.specifics`.
     */
    attributes: jsonb("attributes").$type<Record<string, string>>(),

    // Product identifiers — the only sound basis for matching one unit across
    // sellers and marketplaces. Never match on title phrases.
    model: text("model"),
  ...
  (t) => [
    ...
    index("scan_listing_variant_upc_idx").on(t.upc),
    index("scan_listing_variant_ean_idx").on(t.ean),
    index("scan_listing_variant_gtin_idx").on(t.gtin),
    index("scan_listing_variant_isbn_idx").on(t.isbn),
    index("scan_listing_variant_mpn_idx").on(t.mpn),
  ]
);

/** Listing sales measurements — one row per observed change, not per scan. */
...
/**
 * Price and stock history for one sellable unit — one row per observed
 * **change**, never one per scan.
 *
 * A scan that finds a variant at the price it already held writes nothing, so
 * every row here is a real transition and a price chart is the rows in order.
 * The cost of that is what absence means: a gap is "unchanged", not
 * "unobserved". `scan_listing.last_scanned_at` is the record of when the
 * listing was actually looked at.
 *
 * Reconciliation writes a final row when it marks a unit `removed`, so a
 * unit's history ends with the state it disappeared in rather than trailing
 * off at its last price.
 */
export const scanListingVariantSnapshot = pgTable(
  "scan_listing_variant_snapshot",
  {
    ...
    variantId: text("variant_id")
      .notNull()
      .references(() => scanListingVariant.id, { onDelete: "cascade" }),
    /** Observed price in integer cents. */
    price: integer("price"),
    currency: text("currency"),
    status: text("status")
      .$type<"in_stock" | "out_of_stock" | "removed">()
  ...
  (t) => [
    index("scan_listing_variant_snapshot_history_idx").on(
      t.variantId,
      t.createdAt,
      t.id
    ),
    check(
      "scan_listing_variant_snapshot_status_check",
      sql`${t.status} IN ('in_stock', 'out_of_stock', 'removed')`
    ),
    check("scan_listing_variant_snapshot_price_check", sql`${t.price} >= 0`),
  ]
);
...
export const scanListingVariantRelations = relations(
  ...
  ({ many, one }) => ({
    ...
    snapshots: many(scanListingVariantSnapshot),
  })
);

export const scanListingVariantSnapshotRelations = relations(
  scanListingVariantSnapshot,
  ({ one }) => ({
    variant: one(scanListingVariant, {
      fields: [scanListingVariantSnapshot.variantId],
      references: [scanListingVariant.id],
    }),
  })
);
