...
function Sidebar({
  ...
  if (isMobile) {
    return (
      <Dialog.Root onOpenChange={setOpenMobile} open={openMobile}>
        <Dialog.Portal>
          <Dialog.Backdrop
            className="fixed inset-0 z-20 bg-black/20 transition-opacity duration-300 ease-out data-ending-style:opacity-0 data-starting-style:opacity-0 supports-backdrop-filter:backdrop-blur-xs"
            data-slot="sidebar-backdrop"
          />
          <Dialog.Popup
            className={cn(
              "fixed inset-x-0 top-0 z-30 flex max-h-svh flex-col overflow-y-auto overscroll-contain bg-sidebar-background text-sidebar-foreground shadow-lg",
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="flex size-full flex-col bg-sidebar-background group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:shadow-sm group-data-[variant=floating]:ring-1 group-data-[variant=floating]:ring-sidebar-border"
          data-sidebar="sidebar"
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      {...props}
    >
      <AnimatedMenuIcon className="size-4" isOpen={showClose} />
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "group-data-[collapsible=offcanvas]:translate-x-0 hover:group-data-[collapsible=offcanvas]:bg-sidebar-background group-data-[collapsible=offcanvas]:after:left-full",
...
function SidebarInset({ className, ...props }: ComponentProps<"main">) {
  return (
    <main
      className={cn(
        "relative flex w-full flex-1 flex-col border bg-background md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-0 md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow-sm",
        className
      )}
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left font-medium text-sidebar-foreground/60 text-sm outline-hidden ring-sidebar-ring transition-[width,height,padding] hover:text-sidebar-primary-foreground focus-visible:ring-2 active:text-sidebar-primary-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-active:bg-sidebar-primary data-active:text-sidebar-primary-foreground data-active:shadow-sm data-open:hover:text-sidebar-primary-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! dark:text-sidebar-foreground/70 dark:data-active:text-sidebar-primary-foreground dark:hover:text-sidebar-primary-foreground [&>span:last-child]:truncate [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default: "hover:text-sidebar-primary-foreground",
        outline:
          "bg-sidebar-primary shadow-sm hover:text-sidebar-primary-foreground",
      },
      size: {
        default: "h-8 text-sm",
        sm: "h-7 text-xs",
...
function SidebarMenuSub({ className, ...props }: ComponentProps<"ul">) {
  return (
    <ul
      className={cn(
        "mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-sidebar-foreground/30 border-l px-2.5 py-0.5 group-data-[collapsible=icon]:hidden dark:border-sidebar-foreground/20",
        className
      )}
...
function SidebarMenuSubButton({
  ...
  return useRender({
    ...
    props: mergeProps<"a">(
      {
        className: cn(
          "flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 font-medium text-sidebar-foreground/60 outline-hidden ring-sidebar-ring hover:text-sidebar-primary-foreground focus-visible:ring-2 active:text-sidebar-primary-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[size=md]:text-sm data-[size=sm]:text-xs data-active:text-sidebar-primary-foreground group-data-[collapsible=icon]:hidden dark:text-sidebar-foreground/70 dark:data-active:text-sidebar-primary-foreground dark:hover:text-sidebar-primary-foreground [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-sidebar-primary-foreground",
          className
        ),
      },
      props
