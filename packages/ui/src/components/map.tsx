...
import darkStyle from "@sparkyidea/ui/assets/map/dark.json";
import lightStyle from "@sparkyidea/ui/assets/map/light.json";
...
const defaultStyles = {
  dark: darkStyle as MapLibreGL.StyleSpecification,
  light: lightStyle as MapLibreGL.StyleSpecification,
};
...
// Carries MapRoute's source/layer across a setStyle diff so theme swaps
// don't blink the line. Scoped to the `route-source-` / `route-layer-`
// prefixes on purpose: MapArc (`arc-*`) and MapClusterLayer (`cluster-*`,
// `clusters-*`, `cluster-count-*`, `unclustered-point-*`) aren't covered
// here - extend this if/when those components are used in a themed map.
function preserveRouteLayers(
  prev: MapLibreGL.StyleSpecification | undefined,
  ...
  const routeSources = Object.fromEntries(
    Object.entries(prev.sources ?? {}).filter(
      ([sourceId]) =>
        sourceId.startsWith("route-source-") && !next.sources?.[sourceId]
    )
  );
  const nextLayerIds = new Set((next.layers ?? []).map((layer) => layer.id));
  const routeLayers = (prev.layers ?? []).filter(
    (layer) =>
      layer.id.startsWith("route-layer-") && !nextLayerIds.has(layer.id)
  );

  if (Object.keys(routeSources).length === 0 && routeLayers.length === 0) {
    return base;
  }

  return {
    ...base,
    sources: { ...next.sources, ...routeSources },
    layers: [...(next.layers ?? []), ...routeLayers],
  };
}

function isStyleNotReadyError(error: unknown): boolean {
  ...
  return (
    ...
    message.includes("style is not loaded") ||
    message.includes("load event")
  );
}
...
const MapComponent = forwardRef<MapRef, MapProps>(function MapComponent(
  ...
  useEffect(() => {
    ...
    mapInstance.setStyle(newStyle, {
      diff: true,
      transformStyle: preserveRouteLayers,
    });
...
function MapRoute({
  ...
  const latestRef = useRef({ color, width, opacity, dashArray, coordinates });
  latestRef.current = { color, width, opacity, dashArray, coordinates };
  ...
  useEffect(() => {
    ...
    // Retries "style is still loading" throws from addSource/addLayer so
    // callers (initial mount + styledata retry) don't need to wrap each
    // call. A later styledata event will run this again.
    const ensureRouteLayer = () => {
      try {
        if (!map.getSource(sourceId)) {
          const { coordinates: coords } = latestRef.current;
        ...
        // Style still settling; a later styledata event will retry.
      }
    };
    ...
    return () => {
      map.off("styledata", ensureRouteLayer);
      try {
        if (map.getLayer(layerId)) {
