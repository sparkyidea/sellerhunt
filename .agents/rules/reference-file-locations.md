...
- `apps/app/src/app/auth/[path]/` — login / signup / verification
  (each app mounts its own sign-in; both hit the same `apps/api` auth routes).
...
- Auth client: `packages/auth/src/auth-client.ts` (`buildAuthClient`), instantiated in `apps/app/src/lib/auth-client.ts`.
