...
- **Path alias** — `@/` resolves to the current app's `src/` (`apps/app/src/`). Cross-package imports use `@sparkyidea/*` (UI, dataview) or `@dashseller/*` (db, trpc, auth, marketplace, taxonomy, trigger, env).
