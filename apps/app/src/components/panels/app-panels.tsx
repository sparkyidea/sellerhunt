...
import { useNavigationArea } from "@/components/navigation/navigation-area";
...
import { getNavigationArea, isSettingsPath } from "@/lib/navigation-area";
...
export function AppPanels({ children }: { children: ReactNode }) {
  ...
  // The server layout still gates routes. This also discards retained surfaces
  // if the client session loses admin access while the settings sheet is open.
  if (accessLost) {
    return isSettingsPath(pathname) ? (
      children
    ) : (
      <NotFound homeLink={<Link href="/" />} />
    );
  }
  return (
    <PanelRoot
